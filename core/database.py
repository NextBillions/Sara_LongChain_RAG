    ...
    def create_database(self, database_name, description, db_type, dimension=None):
        ...
        try:
            ...
            if dimension is None:
                dimension = EMBED_MODEL_INFO[self.config.embed_model]["dimension"]
            
            dimension = int(dimension)  # 确保维度是整数
            if dimension <= 0:
            ...
            # 检查是否已存在同名数据库
            if new_database.metaname in self.metaname2db:
                raise ValueError(f"Database {database_name} already exists")

            # 创建向量数据库集合
            try:
                logger.info(f"Creating collection with metaname: {new_database.metaname}")
